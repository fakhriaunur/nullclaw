...
pub const InMemoryLruMemory = struct {
    ...
    pub fn initWithInstanceId(allocator: std.mem.Allocator, max_entries: usize, instance_id: []const u8) !Self {
        _ = instance_id;
        return init(allocator, max_entries);
    }
    ...
    fn implStore(ptr: *anyopaque, key: []const u8, content: []const u8, category: MemoryCategory, session_id: ?[]const u8) anyerror!void {
        ...
        if (self_.entries.count() >= self_.max_entries) self_.evictLru();

        const created_at = try self_.nowTimestamp();
        errdefer self_.allocator.free(created_at);
        const updated_at = try self_.allocator.dupe(u8, created_at);
        errdefer self_.allocator.free(updated_at);

        try self_.entries.put(self_.allocator, try self_.allocator.dupe(u8, storage_key), .{
    ...
    fn implForget(ptr: *anyopaque, key: []const u8) anyerror!bool {
        ...
        defer {
            for (to_remove.items) |storage_key| self_.allocator.free(storage_key);
            to_remove.deinit(self_.allocator);
        }

        var it = self_.entries.iterator();
        while (it.next()) |kv| {
            if (!std.mem.eql(u8, kv.value_ptr.key, key)) continue;
            try to_remove.append(self_.allocator, try self_.allocator.dupe(u8, kv.key_ptr.*));
        }
